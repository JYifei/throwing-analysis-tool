...
def annotate_one_throw(
    ...
    # --- Directional DTW deltas (student - model), per frame ---
    # We will try to load dtw_signed_delta.csv; if missing, auto-generate from align map + model.csv.
    signed_df = None
    signed_delta_path = throw_dir / "dtw_signed_delta.csv"
    align_map_path = throw_dir / "clip_pose_align_map.csv"  # your map file (stu_idx, ref_idx)

    if signed_delta_path.exists():
        try:
            signed_df = pd.read_csv(signed_delta_path)
        except Exception:
            signed_df = None
    else:
        # Auto-generate once (no need to touch fsm_new)
        try:
            if align_map_path.exists():
                # Try to locate reference csv: prefer score_obj meta, fallback to reference/model.csv
                ref_csv = None
                if isinstance(score_obj, dict):
                    meta = score_obj.get("meta", {}) or {}
                    ref_csv = meta.get("reference_csv") or meta.get("ref_csv")  # if you stored it
                if not ref_csv:
                    ref_csv = str((Path(__file__).resolve().parent.parent / "reference" / "model.csv").resolve())
                    if not Path(ref_csv).exists():
                        ref_csv = "reference/model.csv"

                # student csv is always clip.csv inside throw_dir
                stu_csv = str((throw_dir / "clip.csv").resolve())

                # Import utilities from fixed_dtw_comparison (same project)
                from fixed_dtw_comparison import export_signed_deltas_from_align_map

                export_signed_deltas_from_align_map(
                    reference_csv=ref_csv,
                    student_csv=stu_csv,
                    align_map_csv=str(align_map_path),
                    out_csv=str(signed_delta_path),
                    handedness="auto",
                )
                signed_df = pd.read_csv(signed_delta_path)
        except Exception as e:
            print(f"[WARN] signed delta generation failed: {e}")
            signed_df = None
        ...
        if score_obj is not None:
            try:
                ...
                for k, v in feats.items():
                    ...
                    # format text + directional hint (if signed_df is available)
                    suffix = ""
                    if signed_df is not None and frame_idx_in_video < len(signed_df):
                        signed_row = signed_df.iloc[frame_idx_in_video]   # <-- FIX: don't overwrite `row`

                        # angle: use *_delta (deg)
                        if k in angle_feats:
                            d = float(signed_row.get(f"{k}_delta", np.nan))
                            if not np.isnan(d):
                                if d > 0:
                                    suffix = f" (too open, +{abs(d):.1f}°)"
                                elif d < 0:
                                    suffix = f" (too bent, -{abs(d):.1f}°)"

                        else:
                            # distances: prefer *_rel
                            rel = float(signed_row.get(f"{k}_rel", np.nan))

                            # y-dist features: interpret sign as high/low (image y grows downward)
                            if ("_y_dist" in k) and (not np.isnan(rel)):
                                if rel > 0:
                                    suffix = f" (too low, +{abs(rel)*100:.1f}%)"
                                elif rel < 0:
                                    suffix = f" (too high, -{abs(rel)*100:.1f}%)"
                            elif not np.isnan(rel):
                                if rel > 0:
                                    suffix = f" (too large, +{abs(rel)*100:.1f}%)"
                                elif rel < 0:
                                    suffix = f" (too small, -{abs(rel)*100:.1f}%)"


                    if k in angle_feats:
                        text = f"{k}: {fv:.1f} deg{suffix}"
                    else:
                        text = f"{k}: {fv*100:.1f}%{suffix}"
